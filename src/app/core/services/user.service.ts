...
export class UserService {
  ...
  queryUserByRoleIds(donvi_id: number, roleIds: number[], options?: {
    limit?: number,
    order?: string,
    orderby?: string,
    select?: string
  ...
  validateEqualInfo(info: any): Observable<boolean> {
    const cons = [];
    Object.keys(info).forEach(_key => cons.push({
      conditionName: _key,
      condition: OvicQueryCondition.equal,
      value: info[_key]
    }));
  ...
  getListUser(rows:number,  page: number, search?: string): Observable<{ recordsTotal: number, data: User[] }> {
    const conditions: OvicConditionParam[] = [
    ];
