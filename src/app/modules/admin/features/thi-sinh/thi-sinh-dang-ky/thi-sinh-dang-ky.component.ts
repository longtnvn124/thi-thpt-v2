...
export class ThiSinhDangKyComponent implements OnInit {
  ...
  constructor(
    ...
    private truongHocService:DanhMucTruongHocService,
    ...
    this.formSave= this.fb.group({
      user_id:[this._user_id,Validators.required],
  ...
  getDataOrder(){
    this.ordersService.getdata(this._user_id).subscribe({
      next:(data)=>{
        let i =1;
    ...
    if (this.dataOrders.some(f=>f.kehoach_id !== kehoachid)){
      if (this.formSave.valid){
        ...
        this.ordersService.create(formUp.value).subscribe({
          next:(id)=>{
            this.UpOrderMonBylocal(id,this.auth.user.id, this.keHoachThi[0].id,this.hinhthucthiSlect);
            this.getPayment(id);
            this.notifi.isProcessing(false);
