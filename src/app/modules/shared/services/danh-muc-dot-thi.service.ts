...
export class DanhMucDotThiService {
  ...
  load(page: number): Observable<{ recordsTotal: number, data: DmDotThi[] }> {
  ...
  search(page: number, ten: string): Observable<{ recordsTotal: number, data: DmDotThi[] }> {
    const conditions: OvicConditionParam[] = [];
    const fromObject = {
      paged: page,
      limit: this.themeSettingsService.settings.rows,
  ...
  getDataUnlimit(): Observable<DmDotThi[]> {
    const conditions: OvicConditionParam[] = [
      {
